...
import { config, updateConfig, isLocalhost } from "@/utils/config";
  ...
  function renderPage() {
    ...
    case 'appearance':
      return <>
        ...
        <MenuPage
          keys={["background_img", "background_color", "background_video"]}
          menuClick={handleMenuClick} />
      </>;
    ...
    default:
      if (!isLocal) {
        return <div className="rounded-lg shadow-lg bg-white p-4">
          <h2 className="text-xl w-full">접근 제한됨</h2>
          <p className="w-full my-4">이 설정은 로컬호스트에서만 사용할 수 있습니다.</p>
        </div>;
      }
